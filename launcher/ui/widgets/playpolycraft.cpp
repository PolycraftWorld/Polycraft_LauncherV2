    ...
    ui(new Ui::PlayPolycraft)
{
    ...
    connect(APPLICATION->instances().get(), &InstanceList::instancesChanged, this, &PlayPolycraft::updateInstances);
}
...
//    APPLICATION->SendAnalyticsEvent("Launch", ui->instances->currentText(), "test", QVariant(1));
    APPLICATION->launch(APPLICATION->instances()->getInstanceById(ui->instances->currentText()));
}
